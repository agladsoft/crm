...
if __name__ == "__main__":
    ...
    leads: Leads = Leads(input_file_path, output_folder,
                         HEADERS_ENG, LIST_OF_FLOAT_TYPE,
                         LIST_OF_BOOL_TYPE, LIST_OF_INT_TYPE,
                         LIST_OF_DATE_TYPE)
    parsed_data: list = leads.convert_csv_to_dict()
    divided_parsed_data: list = list(leads.divide_chunks(parsed_data, 50000))
    for index, chunk_parsed_data in enumerate(divided_parsed_data):
        for dict_data in chunk_parsed_data:
            leads.change_type(dict_data)
        leads.save_data_to_file(index, chunk_parsed_data)
